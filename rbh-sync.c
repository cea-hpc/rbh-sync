...
/* Helper over rbh_mut_iter_next() to handle/ignore transient errors */
static void *
mut_iter_next(struct rbh_mut_iterator *iterator)
{
...
_convert_iter_next(struct convert_iterator *convert)
{
    ...
    do {
        ...
        fsentry = mut_iter_next(convert->fsentries);
        if (fsentry == NULL)
            return -1;
                 ...
                 struct rbh_mut_iterator *fsevents)
{
    ...
    do {
        struct rbh_fsevent *fsevent = mut_iter_next(fsevents_tee[1]);
...
main(int argc, char *argv[])
{
    ...
    /* insert_fsentries() cannot free fsevents as they are consumed. It has to
    ...
    /* Update `to' */
    do {
        struct rbh_mut_iterator *chunk = mut_iter_next(chunks);
